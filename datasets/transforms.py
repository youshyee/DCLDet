...
class ColorJitter(object):
    def __init__(self,brightness=0, contrast=0, saturation=0, hue=0):
        self.torchjitter=T.ColorJitter(brightness=brightness, contrast=contrast, saturation=saturation, hue=hue)

    def __call__(self, img: PIL.Image.Image, target: dict):
        return self.torchjitter(img), target
...
def random_image_box_translation(img, boxes):
    boxes = boxes.clone()
    im_shape = img.shape
    h = random.randint(int(img.shape[2] / 4),
                       img.shape[2] - int(img.shape[2] / 4))
    w = random.randint(int(img.shape[3] / 4),
                       img.shape[3] - int(img.shape[3] / 4))
...
def random_translate(image, mask, box_scale, box_shift):
    ...
    h = random.randint(int(im_shape[1] / 4),
                       im_shape[1] - int(im_shape[1] / 4))
    w = random.randint(int(im_shape[2] / 4),
                       im_shape[2] - int(im_shape[2] / 4))
    img = torch.nn.functional.interpolate(
        image.unsqueeze(0), (h, w), mode='bilinear')[0]
    m = torch.nn.functional.interpolate(mask.unsqueeze(0).type(
        torch.float), (h, w), mode='nearest')[0].type(torch.bool)
    new_img2 = torch.zeros(im_shape).to(img)
...
def random_crop_and_resize(new_img2, new_mask2, box_scale, box_shift):
    ...
    i, j, h, w = RandomResizedCrop.get_params(
        new_img2, scale=(0.5, 1.), ratio=(1. / 4., 4. / 3.))
    ...
    new_img2 = torch.nn.functional.interpolate(
        new_img2.unsqueeze(0), (orig_h, orig_w), mode='bilinear')[0]
    new_mask2 = torch.nn.functional.interpolate(new_mask2.unsqueeze(0).type(
        torch.float), (orig_h, orig_w), mode='nearest')[0].type(torch.bool)
    i = i / (orig_h - 1)
    j = j / (orig_w - 1)
    f_h = orig_h / h
    f_w = orig_w / w
