...
class DeformableDETR(nn.Module):
    ...
    def __init__(self,
        ...
        Parameters:
            ...
            num_classes: number of object classes without the background class
    ...
    def forward_samples(self, samples):
        ...
        if self.aux_loss:
            out['aux_outputs'] = self._set_aux_loss(
                outputs_class, outputs_coord)  # , outputs_features)
        if self.two_stage:
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, batch_scale, log=True, **kwargs):
    ...
    @torch.no_grad()
    def loss_cardinality(self, outputs, targets, indices, num_boxes, batch_scale, **kwargs):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, batch_scale, **kwargs):
        ...
        losses = {}
        # *(batch_scale.unsqueeze(1)) #.sum() / num_boxes
        losses['loss_bbox'] = loss_bbox
        # print('num_boxes', num_boxes, len(batch_scale))

        loss_giou = 1 - torch.diag(box_ops.generalized_box_iou(
            box_ops.box_cxcywh_to_xyxy(src_boxes),
            box_ops.box_cxcywh_to_xyxy(target_boxes)))
        # print('loss_giou', loss_giou.shape)
    ...
    def loss(self, z1, z2):  # BxNxD
        ...
        labels = torch.arange(0, x.shape[1]).view(
            1, x.shape[1]).repeat_interleave(x.shape[0], 0).cuda()
    ...
    def loss_object_embedding(self, outputs, targets, indices, num_boxes, batch_scale):
        """Compute the losses related to the bounding boxes, the L1 regression loss and the GIoU loss
           targets dicts must contain the key "boxes" containing a tensor of dim [nb_target_boxes, 4]
           The target boxes are expected in format (center_x, center_y, h, w), normalized by the image size.
        """
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes, batch_scale):
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, batch_scale, **kwargs):
        loss_map = {
        ...
        return loss_map[loss](outputs, targets, indices, num_boxes, batch_scale, **kwargs)

    def forward(self, outputs, targets, batch_scale, get_indices=False):
        ...
        losses = {}
        # print(f"losses to use: {self.losses}")
        for loss in self.losses:
            kwargs = {}
            losses.update(self.get_loss(loss, outputs, targets,
                          indices, num_boxes, batch_scale, **kwargs))
            ...
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    ...
                    l_dict = self.get_loss(
                        loss, aux_outputs, targets, indices, num_boxes, batch_scale, **kwargs)
        ...
        if 'enc_outputs' in outputs:
            ...
            for loss in self.losses:
                ...
                if loss == 'labels':
                    ...
                    kwargs['log'] = False
                l_dict = self.get_loss(
                    loss, enc_outputs, bin_targets, indices, num_boxes, batch_scale, **kwargs)
...
class PostProcess(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, outputs, target_sizes, topk=100, istargetsize=True):
        ...
        boxes = torch.gather(
            out_bbox, 1, topk_boxes.unsqueeze(-1).repeat(1, 1, 4))
        ...
        if istargetsize:
            ...
            # boxes is the cxcywh boxes from [0,1]
            pass
